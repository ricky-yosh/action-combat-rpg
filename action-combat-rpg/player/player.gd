...
func handle_slashing_physics_frame(delta: float) -> void:
	...
	attack_cast.deal_damage(user_interface.inventory.get_weapon_damage(), stats.get_crit_chance())

func handle_overhead_physics_frame() -> void:
	if not rig.is_overhead():
		return
	# Stop movement when overhead attack
	velocity.x = 0
	velocity.z = 0
...
func _on_rig_heavy_attack() -> void:
	area_attack.deal_damage(user_interface.inventory.get_weapon_damage(), stats.get_crit_chance())
